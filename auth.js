...
// capitanCompania: la compañía (1-4) de la que es capitán, o null si no lo
// es. Es, igual que superAdmin, un permiso independiente del resto: cuando
// un capitán "entra" en una de las secciones de su compañía (ver
// routes/capitan.js), recibe un token con tenantId/role de ESA sección
// (para reutilizar tal cual las rutas y la pantalla del jefe de sección) y,
// a la vez, conserva capitanCompania — así se sabe que sigue siendo un
// capitán actuando, no el jefe de sección real de esa sección.
...
// Para las rutas de /api/capitan: exige el permiso `capitanCompania` (ser
// capitán de alguna compañía), sea cual sea el tenantId/rol que ese token
// tenga en ese momento (puede que ya haya "entrado" en una sección, ver
// routes/capitan.js).
...
// Para usar DENTRO de rutas de sección (roster, usuarios) que deben seguir
// siendo exclusivas del jefe de sección real, incluso cuando quien está
// actuando ahora mismo en esa sección es un capitán de compañía que ha
// "entrado" en ella (token-swap, ver routes/capitan.js) y por tanto ya pasa
// el auth.requireRole("admin") de esas rutas. Se coloca DESPUÉS de
// requireRole("admin") en la cadena de middlewares.
