...
// sin sección propia, y si no, entre los usuarios de todas las secciones.
// Se usa para el login (todavía no se sabe a qué sección pertenece quien
// intenta entrar) y para impedir DNIs duplicados entre secciones distintas
// al dar de alta un usuario nuevo.
//
// Devuelve { user, tenantId, role, superAdmin } o null.
//   - tenantId es null para un Súper Administrador sin sección propia.
...
function findUserGlobal(dni){
  ...
  if (sa) return { user: sa, tenantId: null, role: null, superAdmin: true };

  const keys = Object.keys(data.tenants);
  for (let i = 0; i < keys.length; i++){
    const t = data.tenants[keys[i]];
    const u = t.users.find(function (x){ return x.dni === dni; });
    if (u) return { user: u, tenantId: keys[i], role: u.role, superAdmin: !!u.superAdmin };
  }
  return null;
}
