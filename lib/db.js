...
function emptyData(){
  return {
    ...
    tenants: {},
    // Reinicios totales para empezar un curso nuevo (panel del Súper
    // Administrador): solo fecha, quién y cuántos registros se borraron,
    // sin datos personales. { fecha, por: { dni, nombre }, secciones, usuarios, alumnos, sanciones, capitanes, jefesEstudios }
    reiniciosHistorial: []
  };
}
...
function normalizeLoaded(raw){
  ...
  if (raw.tenants){
    ...
    merged.reiniciosHistorial = Array.isArray(raw.reiniciosHistorial) ? raw.reiniciosHistorial : [];
    return merged;
  }
