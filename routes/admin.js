...
router.post("/cerrar-curso", auth.requireAuth, auth.requireRole("admin"), async function (req, res){
...
router.post("/usuarios", auth.requireAuth, auth.requireRole("admin"), function (req, res){
...
router.patch("/usuarios/:dni/password", auth.requireAuth, auth.requireRole("admin"), function (req, res){
...
router.patch("/usuarios/:dni/permisos", auth.requireAuth, auth.requireRole("admin"), function (req, res){
  const dni = auth.normalizeDni(req.params.dni);
  const user = req.db.users.find(function (u){ return u.dni === dni; });
  if (!user) return res.status(404).json({ error: "No encontrado." });
...
router.delete("/usuarios/:dni", auth.requireAuth, auth.requireRole("admin"), function (req, res){
  const dni = auth.normalizeDni(req.params.dni);
  if (dni === req.user.dni){
